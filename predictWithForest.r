...
set.seed(123)
fit1 = ranger(
  Category ~ DayOfWeek + PdDistrict + year + month + hour + freq + loc,
  train,
  num.threads = 8,
  num.trees = 300,
  classification = T
...
nrow(test[is.na(test$Category), ])
test
test$Category = pre1$predictions
levels(test$Category)
View(submission_final)
write.csv(submission,"sub1.csv")
sub1 = data.frame(test$Id, test$Category)
...
crime = data.frame(Id = c(1:nrow(crime)), Name = crime)
crime
for (i in crime[, 2]) {
  submission[, i] = 0
...
foreach(i=1:cores, .combine='rbind') %dopar%
  {
  for (i in 545963:nrow(sub1)) {
  for (j in 2:ncol(submission)) {
    if (sub1[i, 2] == colnames(submission)[j]) {
       print(paste(i, j, sub1[i, 2] == colnames(submission)[j], sep = " "))
      submission[i, j] = 1
    }
  }
}
